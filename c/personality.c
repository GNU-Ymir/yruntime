// This file was written using libphobos as an example
// It is greatly inspired from it
...
unsigned int _yrt_exc_size_of_encoded_value (unsigned char encoding) {
...
_Unwind_Ptr _yrt_exc_base_of_encoded_value (unsigned char encoding, struct _Unwind_Context* context) {
    if (encoding == DW_EH_PE_omit) return 0;
...
_Unwind_Ptr _yrt_exc_read_encoded_value_with_base (unsigned char encoding, _Unwind_Ptr base, const char** p) {
    const char** psave = p;
...
_Unwind_Ptr _yrt_exc_read_encoded_value (struct _Unwind_Context* context, unsigned char encoding, const char** p) {
					...
					int* handler)
{
    ...
    unsigned char LPStartEncoding = (unsigned char) *p++;
