 ...
 */
uint8_t _yrt_i_map_entry_insert (_yrt_map_content_t * data, _yrt_map_entry_t * en, uint64_t hash, uint8_t * key, uint8_t * value);

/**
 * Allocate a map entry, bump-allocated out of data's entry slab (see _map_entry_alloc in map.c)
 */
void _yrt_i_map_create_entry (_yrt_map_content_t * data, _yrt_map_entry_t ** en, uint64_t hash, uint8_t * key, uint8_t * value);
 ...
 * @info: unlike _yrt_i_map_copy_entries, this does not allocate new entry nodes - the
 *   existing nodes are relinked into result's bucket array. Only valid when old and
 *   result are the same logical map being resized in place (e.g. from _yrt_i_map_fit),
