...
_yrt_slice_ str_create_len (const char * data, uint64_t len) {
    _yrt_slice_ ret;
    memset (&ret, 0, sizeof (_yrt_slice_));
...
_yrt_slice_ str_copy_len (const char *data, uint64_t len) {
    _yrt_slice_ result;
...
_yrt_slice_ str_empty () {
    _yrt_slice_ str;
    memset (&str, 0, sizeof (_yrt_slice_));
