...
_yrt_slice_ _yrt_demangle_symbol (char * data, uint64_t len) {
    ...
    while (1) {
		...
		if (nb != 0) {
			if (i != 0) {
				_yrt_slice_ tmp = str_create_len ("::", len);
				_yrt_append_slice (&ret, &tmp, 1);
			}

			_yrt_slice_ tmp = str_create_len (data + current, nb);
			_yrt_append_slice (&ret, &tmp, 1);
			current += nb;
			i += 1;
		} else break;
    ...
    if (data [current] == 'F') {
		_yrt_slice_ tmp = str_create_len (" (...)", 6);
		_yrt_append_slice (&ret, &tmp, 1);
    } 
...
_yrt_slice_ _yrt_mangle_path (_yrt_slice_ data) {
    _yrt_slice_ str = str_empty ();
    int current = 0;
    int start = 0;
    int i = 0;
    while (i < data.len) {
		if (((uint8_t*) data.data) [i] == ':') {
			_yrt_slice_ tmp = str_from_int (current);
