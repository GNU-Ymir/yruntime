...
_yrt_exc_thread_stack_t _yrt_i_global_stack;

pthread_mutex_t _yrt_exc_mutex;

void _yrt_exc_init () {
    _yrt_thread_mutex_init (&_yrt_exc_mutex, NULL);
    _yrt_i_global_stack.id = pthread_self ();
    _yrt_i_global_stack.next = NULL;
    _yrt_i_global_stack.stack = NULL;
}
 ...
 */
_yrt_exc_thread_stack_t* _yrt_i_exc_get_thread_stack_current (pthread_t id, _yrt_exc_thread_stack_t * current) {
    while (current != NULL) {
...
_yrt_exc_thread_stack_t* _yrt_i_exc_insert_thread (pthread_t id) {
    ...
    _yrt_exc_thread_stack_t* current = &_yrt_i_global_stack;
    while (current != NULL) {
 ...
 */
void _yrt_i_exc_remove_thread (_yrt_exc_thread_stack_t* stack) {
    if (stack != &_yrt_i_global_stack) {
        _yrt_thread_mutex_lock (&_yrt_exc_mutex);
 ...
 */
_yrt_exc_thread_stack_t* _yrt_i_exc_get_thread_stack (pthread_t id) {
    _yrt_thread_mutex_lock (&_yrt_exc_mutex);
    _yrt_exc_thread_stack_t* got = _yrt_i_exc_get_thread_stack_current (id, &_yrt_i_global_stack);
    _yrt_thread_mutex_unlock (&_yrt_exc_mutex);

    if (got == NULL) {
        return _yrt_i_exc_insert_thread (id);
    }
 ...
 */
_yrt_exception_header_t* _yrt_i_exc_create_header (void* object, _yrt_exc_thread_stack_t* stack) {
 ...
 */
void _yrt_i_exc_free_header (_yrt_exception_header_t* head, _yrt_exc_thread_stack_t* stack) {
    ...
    if (stack-> stack == NULL) {
        _yrt_i_exc_remove_thread (stack);
    }
}
 ...
 */
void _yrt_i_exc_push (_yrt_exception_header_t* e, _yrt_exc_thread_stack_t * stack) {
    e-> next = stack-> stack;
    stack-> stack = e;
}
 ...
 */
_yrt_exception_header_t* _yrt_i_exc_pop (_yrt_exc_thread_stack_t* stack) {
...
void _yrt_i_exc_panic_exception (_yrt_exception_header_t* eh)
{
...
_yrt_exception_header_t* _yrt_i_to_exception_header (struct _Unwind_Exception* exc) {
 ...
 */
void _yrt_i_exc_exception_cleanup (_Unwind_Reason_Code code, struct _Unwind_Exception* exc) {
    _yrt_exception_header_t* eh = _yrt_i_to_exception_header (exc);
    if (code != _URC_FOREIGN_EXCEPTION_CAUGHT && code != _URC_NO_REASON) {
        _yrt_i_exc_panic_exception (eh);
    }

    _yrt_exc_thread_stack_t* stack = _yrt_i_exc_get_thread_stack (eh-> thread_id);
    _yrt_i_exc_free_header (eh, stack);
}


void _yrt_exc_throw (char *file, char *function, unsigned line, void* data) {
    _yrt_exc_thread_stack_t* stack = _yrt_i_exc_get_thread_stack (pthread_self ());
    _yrt_exception_header_t* eh = _yrt_i_exc_create_header (data, stack);
    ...
    _yrt_i_exc_push (eh, stack);

    eh-> unwindHeader.exception_cleanup = &_yrt_i_exc_exception_cleanup;
...
void* _yrt_exc_begin_catch (struct _Unwind_Exception* unwindHeader) {
    _yrt_exc_thread_stack_t* stack = _yrt_i_exc_get_thread_stack (pthread_self ());
    _yrt_exception_header_t* header = _yrt_i_to_exception_header (unwindHeader);
    void* object = header-> object;

    // Something went wront when stacking headers
    if (header != _yrt_i_exc_pop (stack)) {
        _yrt_i_exc_terminate ("Catch error", __FILE__, __FUNCTION__, __LINE__);
    }
